<?php
 ...
 $labelLevel[$i] = $i;
    ...
    //         if($j == 0)
    //             $json2 .= '{"score" : "'.$games[$i+$j*10-1]->score_level.'", "duree" : "'.$games[$i+$j*10-1]->score_level'".'"}';
    //             'duree' => $games[$i+$j*10-1]->duree_level
// $tabJson['score'][] = $games[$i+$j*10-1]->score_level;
// $dureeLevel['duree'][] = $games[$i+$j*10-1]->duree_level;
// }
// ));
    ...
    // dd($dureeLevelJson[0]);
    // $contenuJson = json_encode($tabJson);
    // dd($contenuJson);
    for($j=0; $j<$nbParties; $j++){
        for($i=1; $i<=10; $i++){
            $scoreLevel[] = $games[$i+$j*10-1]->score_level;
            $dureeLevel[] = $games[$i+$j*10-1]->duree_level;
        }
        $scoreTotal[$j] = $scoreLevel;
        $scoreJson[$j] = json_encode($scoreTotal[$j]);
        $dureeTotal[$j] = $dureeLevel;
        $dureeJson[$j] = json_encode($dureeTotal[$j]);
        $scoreLevel = [];
        $dureeLevel = [];
        // $scoreTotal[$j] .= $scoreLevel;

    }
    // dd(json_decode($dureeJson[1]));
        ...
        var donneesParNiveaux = {
            ...
            datasets: [{
                ...
                <?php for($j=0; $j<$nbParties; $j++) : ?>
                    <?= 'data: ['; ?>
                    <?php for($i=1; $i<=10) : ?>
                        <?= ($i==1) ? $scoreTotal[$j][$i] : ', '.$scoreJson[$j]; ?>
                    <?php endfor; ?>
                    <?= '],}'; ?>
                    <? if($j < $nbParties -1) : ?>
                        <?= ', {'; ?>
